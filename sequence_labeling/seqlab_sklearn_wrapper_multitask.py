...
class OppSequenceLabelerMultitask(SklearnTransformerBase):
    ...
    def __init__(self, empty_label_ratio=0.2, loss_freq_weights=False, task_importance=None, **kwargs):
    ...
    def _calc_dset_stats(self, verbose=False):
        ...
        num = sum([len(self._raw_train[label]) for label in TASK_LABELS])
    ...
    def _get_narrative_tasks(self):
        ...
        '''
        return [
            Task(id=TASK_INDICES[task_id], name=None, num_labels=3, type="token_classification")
    ...
    def _init_model(self):
        ...
        self.model = MultiTaskModel(self._hf_model_label, self._get_narrative_tasks(), task_weights=self._task_weights)
        ...
        for label, raw_dset in raw_dset_per_label.items(): # merge datasets as pandas dataframes
            for split in dset_splits:
                if datasets_df[split] is None:
                    ...
                    datasets_df[split] = datasets_df[split].append(raw_dset[split].to_pandas())
...
def test_sklearn_wrapper(lang='en'):
    ...
    seq_lab = OppSequenceLabelerMultitask(num_train_epochs=0.001, empty_label_ratio=0.1, hf_model_label='bert-base-cased', lang=lang, eval=None)
    seq_lab.fit_(docs)

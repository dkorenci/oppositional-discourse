...
from data_tools.classif_data.data_utils import create_classif_dataset, LBL_CRITIC, LBL_CONSP
from data_tools.classif_data.spancateg_classif_data import create_spancateg_classif_dataset
...
def load_classification_dataset(lang, experim_type):
    '''
    :return: list of texts, list of classes, list of text_ids
    '''
    if experim_type == 'critic-conspi':
        return create_classif_dataset(lang, classes=[LBL_CRITIC, LBL_CONSP], output='sklearn')
    elif experim_type.startswith('spancateg-') and len(experim_type) == len('spancateg-X'):
        span_category = experim_type[-1]
        return create_spancateg_classif_dataset(lang, span_category)
    else: raise ValueError(f'Unknown experiment type: {experim_type}')

def run_classif_crossvalid(lang, model_label, model_params, experim_type='critic-conspi', num_folds=5,
    ...
    texts, classes, txt_ids = load_classification_dataset(lang, experim_type)
...
# parameters for SklearnTransformerBase
HF_CORE_HPARAMS = {
    'learning_rate': 2e-5,
    'num_train_epochs': 3,
    'warmup': 0.1,
...
def run_classif_experiments(lang, num_folds, rnd_seed, test=False, experim_label='label',
                            pause_after_fold=0, pause_after_model=0, max_seq_length=256,
                            experim_type='critic-conspi', model_list=None):
    '''
    :param experim_type: 'critic-conspi' or 'spancateg-X' where X is the one-letter label of the span category
    :return:
    '''
    ...
    logger.info(f'RUNNING classif. experiments: type={experim_type} lang={lang.upper()}, num_folds={num_folds}, '
...
def run_all_critic_conspi(seed=3561467):
    for lang in ['en', 'es']:
        run_classif_experiments(lang=lang, num_folds=5, rnd_seed=seed, test=None, max_seq_length=256,
                                experim_type='critic-conspi', pause_after_fold=3, pause_after_model=7)

def run_all_spancateg(categs, seed=3561467):
    for lang in ['en', 'es']:
        for spancateg in categs:
            run_classif_experiments(lang=lang, num_folds=5, rnd_seed=seed, test=None, max_seq_length=256,
                                    experim_type=f'spancateg-{spancateg}', pause_after_fold=3, pause_after_model=7)

if __name__ == '__main__':
    run_all_critic_conspi()
    run_all_spancateg(categs=['O', 'F', 'P', 'V', 'E', 'A'])
